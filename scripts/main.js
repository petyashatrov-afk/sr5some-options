...
function registerUnifiedAddDamage() {
    ...
    if (originalAddDamage) {
        SR5Actor.prototype.addDamage = async function(damageData, ...extraArgs) {
            ...
            // 3. Process Elemental trigger (Post-damage) - Pass complete damageData object instead of damageData.value
                ...
                if (!isPeriodicOrElemental) {
                    ...
                    if (element && ELEMENTAL_CONFIG[element] && damageData.value > 0) {
                        ...
                        try {
                            switch(element) {
                                case 'acid':
                                    await handleAcidDamage(this, damageData);
                                    break;
                                case 'cold':
                                    await handleColdDamage(this, damageData);
                                    break;
                                case 'electricity':
                                    await handleElectricityDamage(this, damageData);
                                    break;
                                case 'fire':
                                    await handleFireDamage(this, damageData);
                                    break;
                            }
                        } catch (err) {
