...
async function applyAttackPenalty(testInstance) {
  ...
  const label = game.i18n.format('SR5_SOME_OPTIONS.Speed.TargetLabel', { distance: distance.toFixed(2) }) ||
                `Штраф за скорость цели (${distance.toFixed(2)} м)`;
...
async function applyDefensePenalty(testInstance) {
  ...
  const label = game.i18n.format('SR5_SOME_OPTIONS.Speed.DefenderLabel', { distance: distance.toFixed(2) }) ||
                `Штраф за скорость защитника (${distance.toFixed(2)} м)`;
